...
public class CommandService3 {
	...
	public StringBuilder getLogsBydebug1(LogRead logRead) {
		...
		try {
			...
			while ((sCurrentLine = br.readLine()) != null) {
				...
				if (sCurrentLine.contains("20 Mar 15:18")) {
	...
	public StringBuilder getCount1() {
		...
		try {
			...
			while ((sCurrentLine = br.readLine()) != null) {
				// System.out.println(sCurrentLine);
				if (sCurrentLine.contains("21 Mar 15:08")) {
	...
	public String runcommandAndReadHtml(Command command) {
		BufferedReader br;
		String sCurrentLine;
		String inner = "";
		...
		try {
			Process pr = rt.exec(logs.getMvnCommand(), null, file);
			pr.waitFor();
			File folder = new File(command.getLocation() + "\\target\\Destination");
			...
			for (int i = 0; i < listOfFiles.length; i++) {
				...
				if (arr[i].endsWith(".html")) {
					...
					//output.append("file name is " + arr[i] + "= ");
					while ((sCurrentLine = br.readLine()) != null) {
						output.append(sCurrentLine);
						System.out.println(sCurrentLine);
						output.append("\r\n");
					}
					//System.out.println("Till here this " + arr[i] + " data");
				}
				else if (arr[i].endsWith(".css")) {
					//System.out.println(arr[i]);
					br = new BufferedReader(new FileReader(folder + "\\" + arr[i]));

					//output1.append("file name is " + arr[i] + "= ");
					while ((sCurrentLine = br.readLine()) != null) {
